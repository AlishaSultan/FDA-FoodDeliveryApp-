...
const AddtoCart = ({ route, navigation }) => {
  ...
  const { item, onOrderPlaced } = route.params;
  //console.log(item)
  ...
  const OrderPlace = async () => {
    ...
    if (uid) {
      const userOrdersCollection = firestore()
        .collection("orders")
        .doc(uid)
        .collection("user_orders");
      const cartItemsRef = firestore().collection("cartItems").doc(uid);
      const cartCountRef = firestore().collection("cartCount").doc(uid);

      try {
        // Check if the same order already exists
        const existingOrder = await userOrdersCollection
          .where("restaurant", "==", item.restaurant)
          .where("name", "==", item.name)
          .where("totalPrice", "==", item.totalPrice)
          .where("Preference", "==", item.selectedPreferenceNames)
          .where("quantity", "==", item.quantity)
          .get();

        if (!existingOrder.empty) {
          // Order already exists, show a message or take appropriate action
          Toast.show({
            type: "error",
            text1: t("DUPLICATED_ORDER"),
            visibilityTime: 4000,
            autoHide: true,
          });
          return;
        ...
        // Add the order details to the user's document in the 'orders' collection
        const orderDocRef = await userOrdersCollection.add({
          instruction: instruction,
          deliveryOption: deliveryOption,
          restaurant: item.restaurant,
          name: item.name,
          image: item.image,
          totalPrice: item.totalPrice + 70,
          Preference: item.selectedPreferenceNames,
          quantity: item.quantity,
          timestamp: firestore.FieldValue.serverTimestamp(),
        });

        // Fetch the orderId from the document snapshot
        const orderId = orderDocRef.id;

        // Fetch the current state of cartItems from Firestore
        const cartItemsDoc = await cartItemsRef.get();
        const currentCartItems = cartItemsDoc.data()?.items || [];

        // Check if the cart is not empty before updating
        if (currentCartItems.length > 0) {
          ...
          await cartItemsRef.set({ items: updatedCartItems }, { merge: true });
        }

        // Update the cart count in the cartCount collection
        const currentCartCountDoc = await cartCountRef.get();
        const currentCartCount = currentCartCountDoc.data()?.count || 0;

        if (currentCartCount > 0) {
          await cartCountRef.set(
            { count: currentCartCount - 1 },
            { merge: true }
          );
        }

        // Navigate to OrderConfirmed screen with orderId parameter
        navigation.navigate("OrderConfirmed", { orderId });

        // ... (add any additional logic after a successful order placement)
      } catch (error) {
    ...
    <View style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.view1}>
          <Text style={styles.text1}>{item.restaurant}</Text>
        </View>
        ...
        <View style={styles.view4}>
          ...
          <View>
            ...
            <View style={styles.view6}>
              <Text style={styles.text12}>{item.productDetails}</Text>
            </View>
          </View>

          <View>
            {item.selectedPreferenceNames.length > 0 ? (
              <Text style={styles.text9}>
                {item.selectedPreferenceNames.map((preferenceName, index) => (
                  <Text
                    style={[
                      styles.text10,
                  ...
                  >
                    {preferenceName}
                    {index !== item.selectedPreferenceNames.length - 1 && "\n"}
                  </Text>
                ))}
              </Text>
            ) : (
        ...
        <View style={styles.view7}>
          <Text style={styles.text11}>
            {t("TOTAL_PRICE")}: Rs {item.totalPrice}
          </Text>
        </View>
        ...
        <Text style={styles.text17}>
          {t("TOTAL_PRICE")}: Rs {(parseFloat(item.totalPrice) + 70).toFixed(2)}
        </Text>
